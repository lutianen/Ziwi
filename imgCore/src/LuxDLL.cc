...
void 
LuxEndianSwap(unsigned char *pData,
                   uint64_t startIndex,
                   uint64_t length)
{
...
/// @brief Get the 8-bits image data from origianl image(8-bits, 12-bits, 16-bits)
              ...
              unsigned char *outputImg)
{
    ...
    case 12:
    {
        try
        {
            ...
            if (highZero)
            {
                ...
                {
                    outputImg[k] = (orgiImg[i] << 4u) + (orgiImg[i + 1] >> 4u);
                    outputImg[k + 1] = (orgiImg[i + 2] << 4u) + (orgiImg[i + 3] >> 4u);
                    outputImg[k + 2] = (orgiImg[i + 4] << 4u) + (orgiImg[i + 5] >> 4u);
                    outputImg[k + 3] = (orgiImg[i + 6] << 4u) + (orgiImg[i + 7] >> 4u);
                    k += 4;
                }
            }
            ...
            else
            {
                for (int i = 0; i < length; i += 3)
                {
                    ...
                    outputImg[k++] = ((orgiImg[i + 1] << 4u) & 0xFF) + (orgiImg[i + 2] >> 4u);

                    // outputImg[k + 2] = orgiImg[i + 3];
                    // outputImg[k + 3] = (orgiImg[i + 4] << 4u) + (orgiImg[i + 5] >> 4u);

                    // outputImg[k + 4] = orgiImg[i + 6];
                    // outputImg[k + 5] = (orgiImg[i + 7] << 4u) + (orgiImg[i + 8] >> 4u);

                    // outputImg[k + 6] = orgiImg[i + 9];
                    // outputImg[k + 7] = (orgiImg[i + 10] << 4u) + (orgiImg[i + 11] >> 4u);
                }
            }
            break;
        }
    ...
    case 16:
    {
        try
        {
            if (highZero)
            {
                std::cerr << "The depth of bit don't match HIGH BIT!" << std::endl;
                ::fflush(stderr);
                return 0;
            }
                      ...
                      int type)
{
    ...
    if (fileFormat == ImageFileType::raw)
    {
        ...
        if (temp.compare(temp.length() - suffix.length(), suffix.length(), suffix) != 0)
        {
            outFileStr += suffix;
        }
        // else {
 ...
 */
long long
LuxWriteImageIntoFile(uint16_t *imgData,
                      const char *outFileName,
                      ImageFileType fileFormat,
                      unsigned long long length,
                      int width,
 ...
 */
inline long long
LuxWriteImageIntoFileExternTo16(uint16_t *imgData,
                      const char *outFileName,
                      ImageFileType fileFormat,
                      unsigned long long length,
                      int width,
                      ...
                      int type)
{
    ...
    if (fileFormat == ImageFileType::raw)
    {
        ...
        if (temp.compare(temp.length() - suffix.length(), suffix.length(), suffix) != 0) {
            ...
            outFileStr = outFileStr.substr(0, _last) + std::string("_ext16.raw");
        }
        ...
        // Write image data into file
        // LuxEndianRevert(reinterpret_cast<unsigned char*>(imgData), length * 2, 16, 
                  ...
                  unsigned char *dst)
{
    ...
    if (length != static_cast<long long int>(width * height * channel))
    {
        std::cout << "width or height or bpp or channel are wrong!!!" << std::endl;
        return -1;
    }

    if (alpha > 0 && alpha < 10)
    {
        std::cout << "Beta is supported with ranging from -10 to 10!!!" << std::endl;
        return -1;
    }
...
LuxDrawHist(const char* inputFileName_, 
            ...
            const char* outFileName)
{
    if (width < 0 || height < 0 || imReadType < 0 || histImgHeight < 0 || histImgWidth < 0) {
    ...
    cv::Mat image;
    if (inputFileName.compare(inputFileName.length() - suffix.length(), suffix.length(), suffix) == 0)
    {
        std::ifstream fin;
    ...
    if (1 == dims) 
    {
        ...
        /// 
		cv::Mat histImage = cv::Mat::zeros(histImgHeight, histImgWidth, CV_8UC3);
		...
		for (int i = 1; i < numbins; i++) {
			cv::line(histImage,
                cv::Point(bin_w*(i - 1), histImgHeight - cvRound(hist.at<float>(i - 1))),
                cv::Point(bin_w*(i), histImgHeight - cvRound(hist.at<float>(i))),
        ...
        //         writeImageIntoFile(histImage.data, outFileName, ImageFileType::raw,
        //             histImage.size().width * histImage.size().height * histImage.channels());
        long long len =
            LuxWriteImageIntoFile(reinterpret_cast<unsigned char *>(histImage.data),
                                  outFileName, ImageFileType::raw,
                                  histImage.size().width * histImage.size().height * histImage.channels(),
                                  histImage.size().width,
                                  histImage.size().height,
                                  CV_8UC3);
        return len;
    }
    /// 3 通道图 - 彩色图
    else if (3 == dims)
    {
        ...
        cv::Mat histImage(histImgHeight, histImgWidth, CV_8UC3, cv::Scalar(20, 20, 20));
        ...
        {
            cv::line(histImage, 
                cv::Point(binStep * (i - 1), histImgHeight - cvRound(b_hist.at<float>(i - 1))),
                cv::Point(binStep * (i), histImgHeight - cvRound(b_hist.at<float>(i))),
                cv::Scalar(255, 0, 0));
            cv::line(histImage, 
                cv::Point(binStep * (i - 1), histImgHeight - cvRound(g_hist.at<float>(i - 1))),
                cv::Point(binStep * (i), histImgHeight - cvRound(g_hist.at<float>(i))),
                cv::Scalar(0, 255, 0));
            cv::line(histImage, 
                cv::Point(binStep * (i - 1), histImgHeight - cvRound(r_hist.at<float>(i - 1))),
                cv::Point(binStep * (i), histImgHeight - cvRound(r_hist.at<float>(i))),
                cv::Scalar(0, 0, 255));
        }
    ...
    case 12:
    {
        try
        {
            ...
            if (highZero)
            {
                for (int i = 0; i < length; i += 8)
                {
                    outputImg[k++] = ((orgiImg[i] << 8) & 0xFF00) + (orgiImg[i + 1] & 0x00FF);
                    outputImg[k++] = ((orgiImg[i + 2] << 8) & 0xFF00 ) + (orgiImg[i + 3] & 0x00FF);

                    outputImg[k++] = ((orgiImg[i + 4] << 8) & 0xFF00) + (orgiImg[i + 5] & 0x00FF);
                    outputImg[k++] = ((orgiImg[i + 6] << 8) & 0xFF00 ) + (orgiImg[i + 7] & 0x00FF);
                }
            }
            /// AAAAAAAA AAAABBBB BBBBBBBB
            else
            {
                ...
                {
                    outputImg[k++] = (((orgiImg[i] >> 4) << 8) & 0xFF00) + (((orgiImg[i] << 4) + (orgiImg[i + 1] >> 4)) & 0x00FF);
                    outputImg[k++] = ((orgiImg[i + 1] & 0x0F) << 8) + orgiImg[i + 2];
                    
                    outputImg[k++] = (((orgiImg[i + 3] >> 4) << 8) & 0xFF00) + (((orgiImg[i + 3] << 4) + (orgiImg[i + 4] >> 4)) & 0x00FF);
                    outputImg[k++] = ((orgiImg[i + 4] & 0x0F) << 8) + orgiImg[i + 5];
                }
            }
            break;
        }
                 ...
                 bool highZero)
{
    ...
    if (bpp == 8 || bpp == 12 || bpp == 16) {
        ...
        uint64_t k = LuxParseImageExtendTo16(imgData, length, bpp, highZero, temp);

         k = k > 0 ? LuxWriteImageIntoFileExternTo16(temp, 
                        outRawFileName, 
                        ImageFileType::raw,
                        k, width, height, CV_16UC1)
            : -1;
        delete [] temp;
        return k;
    }
        ...
        try
        {
            ...
            if (highZero)
            {
                for (int i = 0; i < length; i += 4)
                {
                    ...
                    outputImg[k] = ((orgiImg[i] << 8) + orgiImg[i + 1]) * 16;
                    outputImg[k + 1] = ((orgiImg[i + 2] << 8) + orgiImg[i + 3]) * 16;
                    k += 2;
                }
            }
            /// AAAAAAAA AAAABBBB BBBBBBBB
            else
            {
                ...
                {
                    // orgiImg[i][j] / 2^12 * 2^16
                    outputImg[k] = (((orgiImg[i] >> 4) << 8) + ((orgiImg[i] << 4) + (orgiImg[i + 1] >> 4))) * 16;
                    outputImg[k + 1] = (((orgiImg[i + 1] & 0x0F) << 8) + orgiImg[i + 2]) * 16;
                 ...
                 int code)
{
    ...
    if (dataFormat == 1)
    {
        if (bpp == 8 || bpp == 12 || bpp == 16)
        {
            long long validLength = width * height;
            auto* temp = new uint16_t[validLength];
            uint64_t k = LuxParseImageStretchTo16(imgData, length, bpp, highZero, temp);
            (void)k;

            // uint64_t k = LuxParseImageExtendTo16(reinterpret_cast<uint8_t*>(imgData), length, bpp, highZero,
    ...
    else if (dataFormat == 2)
    {
        if (bpp == 8 || bpp == 12 || bpp == 16)
        {
            ...
            delete[] temp;
            return rgb16BitMat.size().height * rgb16BitMat.size().width * rgb16BitMat.channels();
        }
        else
        {
            std::cerr << "bpp Error!" << std::endl;
                 ...
                 int code)
{
    ...
    unsigned long long k = LuxLoadImageDataStretchTo16(imgData, 
            length, dataFormat, width, height, bpp, inChannels, outChannels, 
            outData, isBigEndian, highZero, code);
        ...
        case 12: {
            try {
                ...
                if (highZero) {
                    // DONE Tested
                    for (int i = 0; i < length; i += 4) {
                        outputImg[k++] = ((orgiImg[i] << 4u) & 0xFF) + (orgiImg[i + 1] >> 4u);
                        outputImg[k++] = ((orgiImg[i + 2] << 4u) & 0xFF) + (orgiImg[i + 3] >> 4u);
                    }
                }
                /// AAAAAAAA AAAABBBB BBBBBBBB
                else {
            ...
            try {
                ...
                if (highZero) {
                    for(int i = 0; i < length; i += 2){
                        outputImg[k++] = ((orgiImg[i] << 4) & 0xFF) + (orgiImg[i + 1] >> 4);
                    }
                }
                /// AAAAAAAA AAAAAAAA
                // DONE Tested
        ...
        case 12: {
            try {
                ...
                if (highZero) {
                    // DONE Tested
                    for (int i = 0; i < length; i += 4) {
                        outputImg[k++] = ((orgiImg[i] & 0x07) << 5) + ((orgiImg[i + 1] & 0xF8) >> 3);
                        outputImg[k++] = ((orgiImg[i + 2] & 0x07) << 5) + ((orgiImg[i + 3] & 0xF8) >> 3);
                    }
                }
                /// AAAAAAAA AAAABBBB BBBBBBBB
                else {
                    // DONE Tested
                    for (int i = 0; i < length; i += 3) {
                        outputImg[k++] = ((orgiImg[i] & 0x7F) << 1) + ((orgiImg[i + 1] & 0x80) >> 7);
                        outputImg[k++] = ((orgiImg[i + 1] & 0x07) << 5) + ((orgiImg[i + 2] & 0xF8) >> 3);
                    }
                }
                break;
            } catch (const std::exception &e) {
        ...
        case 16: {
            try {
                if (highZero) {
                    ...
                    for (int i = 0; i < length; i += 4) {
                        outputImg[k++] = ((orgiImg[i] & 0x7F) << 1) + ((orgiImg[i + 1] & 0x80) >> 7);
                        outputImg[k++] = ((orgiImg[i + 2] & 0x7F) << 1) + ((orgiImg[i + 3] & 0x80) >> 7);
                    }
                }
                break;
            } catch (const std::exception &e) {
            ...
            case 12: {
                try {
                    ...
                    if (highZero) {
                        // DONE Tested
                        for (int i = 0; i < length; i += 4) {
                            outputImg[k++] = ((orgiImg[i] & 0x03) << 6) + ((orgiImg[i + 1] & 0xFC) >> 2);
                            outputImg[k++] = ((orgiImg[i + 2] & 0x03) << 6) + ((orgiImg[i + 3] & 0xFC) >> 2);
                        }
                    }
                    /// AAAAAAAA AAAABBBB BBBBBBBB
                    else {
                        // DONE Tested
                        for (int i = 0; i < length; i += 3) {
                            outputImg[k++] = ((orgiImg[i] & 0x3F) << 2) + ((orgiImg[i + 1] & 0xC0) >> 6);
                            //                 000000BB                         BBBBBB00
                            outputImg[k++] = ((orgiImg[i + 1] & 0x03) << 6) + ((orgiImg[i + 2] & 0xFC) >> 2);
                        }
                    }
                    break;
                } catch (const std::exception &e) {
            ...
            case 16: {
                try {
                    if (highZero) {
                        ...
                        for (int i = 0; i < length; i += 4) {
                            outputImg[k++] = ((orgiImg[i] & 0x3F) << 2) + ((orgiImg[i + 1] & 0xC0) >> 6);
                            outputImg[k++] = ((orgiImg[i + 2] & 0x3F) << 2) + ((orgiImg[i + 3] & 0xC0) >> 6);
                        }
                    }
                    break;
                } catch (const std::exception &e) {
            ...
            case 12: {
                try {
                    ...
                    if (highZero) {
                        // DONE Tested
                        for (int i = 0; i < length; i += 4) {
                            outputImg[k++] = ((orgiImg[i] & 0x01) << 7) + ((orgiImg[i + 1] & 0xFE) >> 1);
                            outputImg[k++] = ((orgiImg[i + 2] & 0x01) << 7) + ((orgiImg[i + 3] & 0xFE) >> 1);
                        }
                    }
                    /// AAAAAAAA AAAABBBB BBBBBBBB
                    else {
                        // DONE Tested
                        for (int i = 0; i < length; i += 3) {
                            outputImg[k++] = ((orgiImg[i] & 0x1F) << 3) + ((orgiImg[i + 1] & 0xE0) >> 5);
                            outputImg[k++] = ((orgiImg[i + 1] & 0x01) << 7) + ((orgiImg[i + 2] & 0xFE) >> 1);
                        }
                    }
                    break;
                } catch (const std::exception &e) {
            ...
            case 16: {
                try {
                    if (highZero) {
                        ...
                        for (int i = 0; i < length; i += 4) {
                            outputImg[k++] = ((orgiImg[i] & 0x1F) << 3) + ((orgiImg[i + 1] & 0xE0) >> 5);
                            outputImg[k++] = ((orgiImg[i + 2] & 0x1F) << 3) + ((orgiImg[i + 3] & 0xE0) >> 5);
                        }
                    }
                    break;
                } catch (const std::exception &e) {
            ...
            case 12: {
                try {
                    ...
                    /// AAAAAAAA AAAABBBB BBBBBBBB
                        ...
                        // DONE Tested
                        for (int i = 0; i < length; i += 3) {
                            outputImg[k++] = ((orgiImg[i] & 0x0F) << 4) + ((orgiImg[i + 1] & 0xF0) >> 4);
                            outputImg[k++] = orgiImg[i + 2];
                        }
                    }
                    break;
            ...
            case 16: {
                try {
                    if (highZero) {
                        ...
                        for (int i = 0; i < length; i += 4) {
                            outputImg[k++] = ((orgiImg[i] & 0x0F) << 4) + ((orgiImg[i + 1] & 0xF0) >> 4);
                            outputImg[k++] = ((orgiImg[i + 2] & 0x0F) << 4) + ((orgiImg[i + 3] & 0xF0) >> 4);
                        }
                    }
                    break;
                } catch (const std::exception &e) {
                 ...
                 int code)
{
    ...
    // unsigned long long k = LuxLoadImageData(imgData, length, dataFormat, width, height, bpp, channels, outData, isBigEndian, highZero, code);
    long long k = LuxLoadImageDataEnhanced(imgData, length, dataFormat, 
